...
5. 元音在词首出现，继续使用小写。 anatta 记作：`antVt` 。即首字母沿用 a,ā,i,ī,u,ū,e,o，字中间的元音，以 `A`表示ā長音 , `I`表i  ,  `IA`表ī , `U`表u , `UA`表ū, `E`表e, `O`表o 表示。如 sāsana 编码为 `sAsn` ，套上字体呈现为 s̄sn 。

6. 软颚音 ṅ 只出现在  k, kh, g, gh 之前，与卷舌 ṇ 也永远不会在k,kh,g,gh 之前，因此两者共用编码 `N` 。例：saṅgha 记为是 `sNG`。字体会将 软颚音化入 k 和 g ，变成上方的黑点。vaṇṇa 记为 `vNVN`。

8. ḷ 以 `L` 表示。

9. ṃ 记作 `M` ，显下在右下方，ṃ 只出现在元音之后。

10. 最后五个字母 ḍ `F` , ḍh `Q` , ṭ `W` , ṭh `X` ,ñ `Y`。助记法是 `F` 是 d 之后首个巴利语中没用到的英文本母，然后是`Q`，而t之后还没用到的是`W`，之后是 `X` 。ñ的发音是nYa。

11. 单字如果不是以元音或ṃ 结尾，必须補 V 。例 yad  记作 `ydV` ，mahant 记作 `mhnVtV`。
